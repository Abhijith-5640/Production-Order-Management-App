...
public sealed record GetOrdersQuery(int SectionId, string Trip);
...
public sealed class GetOrdersHandler : IHandler<GetOrdersQuery, GetOrdersResult>
{
    ...
    {
        if (request.SectionId <= 0 || string.IsNullOrWhiteSpace(request.Trip))
            return Error.InvalidInput("section and trip are required");

        try
        {
            var orders = await _orders.GetOrdersAsync(request.SectionId, request.Trip, cancellationToken);
            return new GetOrdersResult(orders);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "GetOrders failed for sectionId {SectionId} trip {Trip}", request.SectionId, request.Trip);
